...
export default function LandingPage() {
  ...
  const tLounge = getTranslations(language).loungeExperience;
  const tWorks = getTranslations(language).howItWorks;
  const tServices = getTranslations(language).services;
  const tBarbers = getTranslations(language).barbers;
  const tJoin = getTranslations(language).joinBanner;
  ...
  return (
    <div className="min-h-screen bg-[#08080a] text-zinc-100 flex flex-col selection:bg-amber-400/30 selection:text-amber-200">
      ...
      <section className="relative pt-12 pb-20 md:pt-20 md:pb-28 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            {/* Left / Main Content */}
            <div className={`lg:col-span-7 space-y-6 text-center ${isArabic ? 'lg:text-right' : 'lg:text-left'}`}>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-extrabold tracking-tight text-white leading-[1.15]">
                {tHero.title1} <br />
                <span className="gold-gradient-text">{tHero.title2}</span>
              </h1>
              ...
              <div className={`flex flex-col sm:flex-row items-center justify-center ${isArabic ? 'lg:justify-start' : 'lg:justify-start'} gap-4 pt-2`}>
                <Link
                  href="/#services"
                ...
                <button
                  onClick={() =>
                    setModalVideo({
                      ...
                      title: `${tNav.brand} • ${tLounge.playerTitle}`,
                      igUrl: 'https://www.instagram.com/reel/DFyFmfsNdVS/',
                    })
                  }
                  className="w-full sm:w-auto px-7 py-4 rounded-2xl text-sm font-bold bg-zinc-900/90 border border-amber-500/40 text-amber-300 hover:text-white hover:bg-zinc-800 hover:border-amber-400 flex items-center justify-center gap-3 transition group shadow-lg cursor-pointer"
                >
                  ...
                  <span>{tHero.ctaFilm}</span>
                </button>
              </div>

              {/* Trust Badges */}
              <div className={`pt-6 border-t border-zinc-800/80 flex flex-wrap items-center justify-center ${isArabic ? 'lg:justify-start' : 'lg:justify-start'} gap-6 text-xs text-zinc-400`}>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>{tHero.badgeNoCards}</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>{tHero.badgeInstantLookup}</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>{tHero.badgeFreeEvery5}</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-5 relative">
              ...
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                >
                  ...
                  <span>{tHero.tabVideo}</span>
                </button>
              </div>

              <div className="relative mx-auto max-w-md">
                {heroTab === 'CARD' ? (
                  /* VIP Loyalty Card Preview */
                  <div className="rounded-3xl bg-gradient-to-b from-[#16161c] to-[#0f0f13] border border-amber-500/30 p-6 sm:p-7 shadow-2xl shadow-black/80 relative overflow-hidden animate-in fade-in duration-300">
                    ...
                    <div className="py-6">
                      <div className="flex items-center justify-between mb-3 text-xs">
                        <span className="text-zinc-400 font-medium">{tHero.stampProgress}</span>
                        <span className="text-amber-400 font-bold font-mono">{tHero.visitsCycle}</span>
                      </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-xs font-bold text-amber-300 block">
                          {tHero.complimentaryTitle}
                        </span>
                        <span className="text-[11px] text-zinc-400 block">
                          {tHero.complimentaryDesc}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-3xl bg-gradient-to-b from-[#16161c] to-[#0f0f13] border border-amber-500/40 p-4 shadow-2xl shadow-black/80 relative overflow-hidden animate-in fade-in duration-300">
                    <div className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-zinc-950 group flex items-center justify-center">
                      ...
                      <button
                        ...
                        className="absolute inset-0 flex flex-col items-center justify-center gap-3 cursor-pointer z-20"
                      >
                      ...
                      {/* Bottom Tag */}
                      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs z-20">
                        <span className="font-serif font-bold text-amber-300">
                          {tNav.brand} {tNav.tagline}
                        </span>
                        <a
                          href={instagramUrl}
                          target="_blank"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="absolute inset-0 z-20 bg-gradient-to-t from-black/80 via-transparent to-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-5">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={togglePlay}
                        className="w-10 h-10 rounded-xl bg-amber-400 text-zinc-950 flex items-center justify-center hover:scale-105 transition cursor-pointer"
                      >
                        {isPlaying ? (
                      ...
                      <button
                        onClick={toggleMute}
                        className="w-10 h-10 rounded-xl bg-zinc-900/80 text-white border border-zinc-700 flex items-center justify-center hover:bg-zinc-800 transition cursor-pointer"
                      >
                        {isMuted ? (
                    ...
                    <button
                      onClick={() =>
                        setModalVideo({
                          src: '/videos/dahab-reel.mp4',
                          title: tLounge.playerTitle,
                          igUrl: 'https://www.instagram.com/reel/DFyFmfsNdVS/',
                        })
                      }
                      className="px-4 py-2 rounded-xl bg-zinc-900/80 hover:bg-zinc-800 text-xs font-bold text-zinc-200 border border-zinc-700 cursor-pointer"
                    >
                      {tLounge.fullScreenHd}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 grid grid-cols-3 gap-3">
              {reels.map((reel, idx) => {
                const reelTrans = tLounge.reels[idx] || {
                  title: 'Dahab Master Cut',
                  tag: '#DahabBarber',
                };
      ...
      <section id="services" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div className="flex flex-wrap gap-2">
            {(['ALL', 'HAIRCUT', 'PACKAGE', 'BEARD', 'TREATMENT'] as const).map((cat) => {
              const label = tServices.categories[cat] || cat;
              return (
                <button
                  key={cat}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredServices.map((service) => {
            const itemTrans = tServices.items[service.id as keyof typeof tServices.items];
            const name = itemTrans?.name || service.name;
            const desc = itemTrans?.desc || service.description;
            const duration = itemTrans?.duration || service.duration;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {INITIAL_BARBERS.map((barber) => {
              const barberTrans = tBarbers.items[barber.id as keyof typeof tBarbers.items];
              const name = barberTrans?.name || barber.name;
              const role = barberTrans?.role || barber.role;
              const specialty = barberTrans?.specialty || barber.specialty;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-xs text-amber-400 font-medium mb-2">{role}</p>
      ...
      <aside className={`fixed bottom-6 ${isArabic ? 'left-6' : 'right-6'} z-40`}>
        <a
          href={instagramUrl}
          target="_blank"
          rel="noopener noreferrer"
        ...
        >
          ...
          <span className="text-xs font-bold hidden sm:inline">{tJoin.floatingIg}</span>
        </a>
      </aside>
